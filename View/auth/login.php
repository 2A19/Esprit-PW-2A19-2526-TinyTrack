...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    // Google login path : a JWT credential was POSTed by Google Identity Services.
    if (!empty($_POST['google_jwt'])) {
        $result = $authCtrl->loginWithGoogle($_POST['google_jwt']);
        if ($result['success']) {
  ...
  <div class="login-card">
    ...
    <div class="form-panel <?= $activeTab==='admin'?'active':'' ?>" id="panel-admin">
      ...
      <form method="POST" novalidate onsubmit="return validerAdmin()">
        ...
        <div class="form-group-p"><label><i class="fas fa-envelope" style="color:#26A69A;"></i> Email</label><input type="text" class="form-control" id="admin_email" name="email" placeholder="Adresse email" value="<?= htmlspecialchars($_POST['email'] ?? '') ?>"><div class="invalid-feedback" id="err_admin_email"></div></div>
        ...
        <div style="text-align:center;margin-top:0.8rem;"><a href="forgot_password.php" style="color:#888;font-size:0.82rem;font-weight:700;text-decoration:none;"><i class="fas fa-key"></i> Mot de passe oublié ?</a></div>
      </form>
    </div>
    ...
    <div class="form-panel <?= $activeTab==='parent'?'active':'' ?>" id="panel-parent">
      ...
      <form method="POST" novalidate onsubmit="return validerParent()">
        ...
        <div class="form-group-p"><label><i class="fas fa-child" style="color:#FF6B9D;"></i> Code Enfant</label><input type="text" class="form-control" id="enfant_id" name="enfant_id" placeholder="TT-XXXX" value="<?= htmlspecialchars($_POST['enfant_id'] ?? '') ?>"><div class="invalid-feedback" id="err_enfant_id"></div></div>
...
async function doFaceLogin() {
  ...
  const r = await FaceAuth.postJSON('/TinyTrack/View/auth/face_login.php', { descriptor: desc });
  if (r.success) {
