...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $result = $authCtrl->requestReset($email);

    if (!$result['success']) {
        $error = $result['error'];
    } else {
        // Build absolute reset URL from the configured app base (see config/mailer.php).
        $resetUrl = APP_BASE_URL . '/View/auth/reset_password.php?token=' . urlencode($result['token']);
        ...
        if ($ok) {
            ...
            $mailFailed = true;
            $error = "L'envoi de l'email a echoue. Reessayez dans quelques instants ou contactez l'administration.";
        }
    }
}
?>
...
<div class="fp-card">
  ...
  <div class="fp-body">
    <?php if ($submitted): ?>
      <div class="success-box">
        ...
        <a href="login.php" class="btn-go-back"><i class="fas fa-arrow-left"></i> Retour au login</a>
      </div>
    <?php else: ?>
      <?php if ($error): ?>
  ...
  <?php if (!$submitted): ?>
  <div class="fp-footer">
    <a href="login.php"><i class="fas fa-arrow-left"></i> Retour au login</a>
  </div>
  <?php endif; ?>
</div>
</body>
