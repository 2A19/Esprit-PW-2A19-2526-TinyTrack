<?php
// Vue passive — données injectées par EvenementController::dashboard()
// Variables : $allEvents, $totalEvents, $totalPrix, $countPlanifie, $countEnCours, $countTermine, $countAnnule, $allRes
$totalRes = count($allRes ?? []);
$countConfirmee = $countEnAttente = $countResAnnulee = $countPaye = 0;
foreach (($allRes ?? []) as $r) {
...
// Stats avis : moyenne globale + top 3 événements
$avisRanking = [];
$totalNotesGlobal = 0; $totalNbAvisGlobal = 0;
foreach ($allEvents as $ev) {
    $avisList = json_decode($ev['avis'] ?? '[]', true) ?: [];
    if (!empty($avisList)) {
        $nb  = count($avisList);
        $moy = round(array_sum(array_column($avisList,'note')) / $nb, 1);
        $totalNotesGlobal += array_sum(array_column($avisList,'note'));
        $totalNbAvisGlobal += $nb;
        $avisRanking[] = ['titre' => $ev['titre'], 'moy' => $moy, 'nb' => $nb];
    }
}
usort($avisRanking, fn($a,$b) => $b['moy'] <=> $a['moy']);
$moyenneGlobale = $totalNbAvisGlobal > 0 ? round($totalNotesGlobal / $totalNbAvisGlobal, 1) : 0;
...
<div class="content-wrapper">
  ...
  <section class="content">
    <div class="container-fluid">
      ...
      <div class="row">
        <div class="col-lg-3 col-6">
          <div class="small-box bg-success">
            ...
            <a href="/TinyTrack/evenements" class="small-box-footer">Voir tout <i class="fas fa-arrow-circle-right"></i></a>
          </div>
        </div>
        <div class="col-lg-3 col-6">
        ...
        <div class="col-lg-3 col-6">
          <div class="small-box bg-warning">
            ...
            <a href="/TinyTrack/evenements" class="small-box-footer">Détails <i class="fas fa-arrow-circle-right"></i></a>
          </div>
        </div>
        <div class="col-lg-3 col-6">
      ...
      <div class="row mb-3">
        <div class="col-md-12">
          <a href="/TinyTrack/evenements/add" class="btn btn-success mr-2"><i class="fas fa-calendar-plus"></i> Nouvel événement</a>
          <a href="/TinyTrack/reservations/add" class="btn btn-info mr-2"><i class="fas fa-ticket-alt"></i> Nouvelle réservation</a>
          <a href="/TinyTrack/reservations" class="btn btn-warning"><i class="fas fa-list"></i> Réservations</a>
        </div>
      </div>
      ...
      <div class="card card-success">
        ...
        <div class="card-body">
          <table class="table table-bordered table-hover table-striped">
            ...
            <tbody>
              ...
              <tr>
                ...
                <td>
                  <a href="/TinyTrack/evenements/edit/<?= $ev['id'] ?>" class="btn btn-sm btn-warning"><i class="fas fa-edit"></i></a>
                  <form method="POST" action="/TinyTrack/evenements/delete/<?= $ev['id'] ?>" style="display:inline" onsubmit="return confirm('Supprimer ?')">
          ...
          <?php if ($totalEvents > 5): ?>
            <div class="text-center mt-2"><a href="/TinyTrack/evenements" class="btn btn-default">Voir tous les <?= $totalEvents ?> événements</a></div>
          <?php endif; ?>
        </div>
      </div>
      ...
      <div class="row">
        <div class="col-lg-3 col-6">
          <div class="small-box bg-info">
            ...
            <a href="/TinyTrack/reservations" class="small-box-footer">Voir tout <i class="fas fa-arrow-circle-right"></i></a>
          </div>
        </div>
        <div class="col-lg-3 col-6">
          <div class="small-box bg-success">
            ...
            <div class="icon"><i class="fas fa-check-circle"></i></div>
            <a href="/TinyTrack/reservations" class="small-box-footer">Détails <i class="fas fa-arrow-circle-right"></i></a>
          </div>
        </div>
        <div class="col-lg-3 col-6">
          <div class="small-box bg-warning">
      ...
      <div class="row mb-3">
        <div class="col-md-12">
          <a href="/TinyTrack/reservations/add" class="btn btn-info mr-2"><i class="fas fa-plus-circle"></i> Nouvelle réservation</a>
          <a href="/TinyTrack/reservations" class="btn btn-default"><i class="fas fa-list"></i> Toutes les réservations</a>
        </div>
      </div>
      ...
      <div class="card card-info">
        ...
        <div class="card-body">
          <table class="table table-bordered table-hover table-striped">
            ...
            <tbody>
              ...
              <tr>
                ...
                <td>
                  <a href="/TinyTrack/reservations/edit/<?= $res['id'] ?>" class="btn btn-sm btn-warning"><i class="fas fa-edit"></i></a>
                </td>
              </tr>
              <?php endforeach; ?>
            </tbody>
          ...
          <?php if ($totalRes > 5): ?>
            <div class="text-center mt-2"><a href="/TinyTrack/reservations" class="btn btn-default">Voir toutes les <?= $totalRes ?> réservations</a></div>
          <?php endif; ?>
        </div>
      </div>

      <!-- ===== AVIS ===== -->
      <h3 class="mt-4 mb-3" style="font-family:'Fredoka One',cursive;color:#2D3436;"><i class="fas fa-star text-warning"></i> Avis & évaluations</h3>

      <div class="row mb-4">
        <div class="col-lg-3 col-6">
          <div class="small-box bg-warning">
            <div class="inner"><h3><?= $moyenneGlobale > 0 ? number_format($moyenneGlobale, 1) . '★' : '—' ?></h3><p>Note moyenne globale</p></div>
            <div class="icon"><i class="fas fa-star"></i></div>
          </div>
        </div>
        <div class="col-lg-3 col-6">
          <div class="small-box bg-info">
            <div class="inner"><h3><?= $totalNbAvisGlobal ?></h3><p>Avis publiés</p></div>
            <div class="icon"><i class="fas fa-comments"></i></div>
          </div>
        </div>
        <div class="col-lg-6">
          <div class="card">
            <div class="card-header bg-white"><h3 class="card-title"><i class="fas fa-trophy text-warning"></i> Top événements</h3></div>
            <div class="card-body p-2">
              <?php if (empty($avisRanking)): ?>
                <p class="text-muted text-center mb-0">Aucun avis pour le moment.</p>
              <?php else: ?>
                <table class="table table-sm mb-0">
                  <?php foreach (array_slice($avisRanking, 0, 3) as $i => $r): ?>
                    <tr>
                      <td style="width:30px;"><?= $i + 1 ?>.</td>
                      <td><strong><?= htmlspecialchars($r['titre']) ?></strong></td>
                      <td class="text-right" style="white-space:nowrap;">
                        <span style="color:#FFB400;"><?= str_repeat('★', round($r['moy'])) ?></span>
                        <strong><?= number_format($r['moy'], 1) ?></strong>
                        <small class="text-muted">(<?= $r['nb'] ?>)</small>
                      </td>
                    </tr>
                  <?php endforeach; ?>
                </table>
