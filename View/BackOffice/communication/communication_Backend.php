...
$dbStats = new CommDatabase();
$pdoStats = $dbStats->connect();

$statTotalConvos = (int) $pdoStats->query("SELECT COUNT(*) FROM conversation")->fetchColumn();
$statActiveConvos = (int) $pdoStats->query("SELECT COUNT(*) FROM conversation WHERE status <> 'archived'")->fetchColumn();
$statTotalMessages = (int) $pdoStats->query("SELECT COUNT(*) FROM chat_message")->fetchColumn();
        ...
        <div class="comm-panel">
          <?php if (!$selectedConversation): ?>
            ...
            <div class="conversation-search-wrap">
              <input type="text" id="conversationSearch" class="form-control conversation-search"
                    ...
                    <tbody>
                      <?php foreach ($conversations as $conversation): ?>
                        ...
                        <tr class="conversation-row" data-href="<?= htmlspecialchars($rowLink) ?>"
                          ...
                          <td>
                            <div class="conversation-main-sub child">
                              <?= htmlspecialchars($rowChildNames !== '' ? $rowChildNames : 'Aucun') ?>
                            </div>
                          </td>
                          <td>
                          ...
                          <td class="row-actions">
                            <button type="button" class="row-menu-trigger" data-row-menu="<?= htmlspecialchars($rowMenuId) ?>"
                              aria-label="Actions">
          ...
          <?php else: ?>
            <div class="chat-header">
              <div>
                ...
                <h2 class="chat-user mt-3">
                  <?= htmlspecialchars($staffName !== '' ? $staffName : 'Conversation') ?>
                </h2>
                <div class="chat-subtext" style="color:#4CAF50;font-weight:800;">
                  <?= htmlspecialchars($childNames !== '' ? $childNames : 'Aucun') ?>
                </div>
              </div>
                    ...
                    <div class="chat-bubble">
                      <?php if ($isFlagged): ?>
                        <div class="message-menu-wrap">
                          <button type="button" class="message-menu-trigger"
                            data-message-menu="back-message-menu-<?= (int) $message->id ?>" aria-label="Options">
                      ...
                      <?php endif; ?>
                      <div class="chat-meta">
                        <?= $isMine ? 'Admin' : htmlspecialchars($message->sender_role) ?>
                      </div>
                      <div>
            ...
            <form method="POST" action="/TinyTrack/Controller/storeMessage.php" class="composer"
              ...
              <div class="composer-wrap">
                ...
                <input type="file" id="composerFiles" name="attachments[]" multiple
                  accept="image/*,.pdf,.doc,.docx,.png,.jpg,.jpeg,.webp" style="display: none;">
...
<script>
  ...
  var adminMsgForm = document.getElementById('adminMsgForm');
  if (adminMsgForm) {
    adminMsgForm.addEventListener('submit', function(e) {
      ...
      var sendBtnIcon = sendBtn.querySelector('i');
      ...
      // Prepare animation
      var originalIconClass = sendBtnIcon.className;
      ...
      .then(data => {
        if (data.success) {
          // Build message HTML
          appendAdminMessage(data.message.body, data.message.created_at, true);

          // If there's a bot reply, append it too (though rare for admin to get bot replies)
          if (data.bot_reply) {
            appendAdminMessage(data.bot_reply.body, data.bot_reply.created_at, false);
          }
          ...
          if (composerFiles) composerFiles.value = '';
          if (composerFilesList) composerFilesList.innerHTML = '';
        }
      })
    ...
    function appendAdminMessage(text, time, isMine) {
      var thread = document.getElementById('chatThread');
      if (!thread) return;

      // Remove empty box if present
      var emptyBox = thread.querySelector('.empty-box');
      ...
      var avatarColor = isMine ? '#4CAF50' : '#FFA726';
      var avatarIcon = isMine ? 'shield-alt' : 'user';
      var metaName = isMine ? 'Admin' : 'Parent';
      ...
      msgDiv.innerHTML = `
        ...
        <div class="chat-bubble">
          <div class="chat-meta">${metaName}</div>
          <div>${text.replace(/\n/g, '<br>')}</div>
