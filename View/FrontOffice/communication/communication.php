...
if (msgForm) {
  msgForm.addEventListener('submit', function(e) {
    ...
    var sendBtn = document.querySelector('.front-send-btn');
    ...
    .then(data => {
      if (data.success) {
        // Build my message HTML
        appendMessage(data.message.body, data.message.created_at, true);

        // If there's a bot reply, build its HTML too
        if (data.bot_reply) {
          appendMessage(data.bot_reply.body, data.bot_reply.created_at, false);
        }

        // Reset input
        document.getElementById('msgBody').value = '';
        document.getElementById('msgBody').style.height = 'auto'; // Reset height if needed
        
        // Reset bot icon style
  ...
  function appendMessage(text, time, isMine) {
    var thread = document.getElementById('frontChatThread');
    if (!thread) return;

    // Remove empty state if present
    var emptyState = thread.querySelector('.text-center.py-5');
    if (emptyState) emptyState.remove();

    var msgDiv = document.createElement('div');
    msgDiv.className = 'front-message ' + (isMine ? 'mine' : '');
    ...
    var avatarColor = isMine ? '<?= $roles[$currentUserRole]["color"] ?? "#999" ?>' : '#4CAF50';
    var avatarIcon = isMine ? '<?= $roles[$currentUserRole]["icon"] ?? "user" ?>' : 'shield-alt';
    var metaName = isMine ? 'Vous' : 'Admin';

    msgDiv.innerHTML = `
      <div class="front-avatar" style="background:${avatarColor};">
        <i class="fas fa-${avatarIcon}"></i>
      </div>
      <div class="front-bubble">
        <div class="front-meta">${metaName}</div>
        <div>${text.replace(/\n/g, '<br>')}</div>
        <div class="front-time">${time}</div>
      </div>
    `;
